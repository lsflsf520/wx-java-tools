 ...
 */
package me.chanjar.weixin.mp.bean.menu.json;
 ...
 * @author Daniel Qian
 */
public class WxMpMenuGsonAdapter implements JsonDeserializer<WxMpMenu> {
 ...
 /* protected JsonObject convertToJson(WxMenuButton button) {
  ...
  @Override
  public WxMpMenu deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
    /*
     * 操蛋的微信
     * 创建菜单时是 { button : ... }
     * 查询菜单时是 { menu : { button : ... } }
     */
	  WxMpConditionalMenu condMenu = new WxMpConditionalMenu();
	  condMenu.setButtons(new ArrayList<WxMenuButton>());
	  
	  WxMpMenu menu = new WxMpMenu();
	  menu.setMenu(condMenu);
    ...
    for (int i = 0; i < buttonsJson.size(); i++) {
      ...
      menu.getMenu().getButtons().add(button);
